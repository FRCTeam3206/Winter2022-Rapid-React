...
public class omnimech extends TimedRobot {
  ...
  public double normalizeStickVals(double i, double max, double min) {
    return 2 * (i - min) / (max - min) - 1;
  }

  public double findMinOrMax(double a, double b, int minOrMax) { // Find the min/max of a value. True = find max, False = find min
    switch(minOrMax) {
      case(1):
          if(a > b) {
            return a;
      ...
      default:
        System.out.println("Please use a valid min/max option. 1 = max, 2 = min");
        return 99.99;
    }
  ...
  public void teleopPeriodic() {
    ...
    xbox_xAxisRStickMAX = findMinOrMax(xbox_xAxisRStickMAX, xbox_xAxisRStick, 1);
    xbox_xAxisRStickMIN = findMinOrMax(xbox_xAxisRStickMIN, xbox_xAxisRStick, 2);
    xbox_yAxisRStickMAX = findMinOrMax(xbox_yAxisRStickMAX, xbox_yAxisRStick, 1);
    xbox_yAxisRStickMIN = findMinOrMax(xbox_yAxisRStickMIN, xbox_yAxisRStick, 2);

    xbox_xAxisLStickMAX = findMinOrMax(xbox_xAxisLStickMAX, xbox_xAxisLStick, 1);
    xbox_xAxisLStickMIN = findMinOrMax(xbox_xAxisLStickMIN, xbox_xAxisLStick, 2);
    xbox_yAxisLStickMAX = findMinOrMax(xbox_yAxisLStickMAX, xbox_yAxisLStick, 1);
    xbox_yAxisLStickMIN = findMinOrMax(xbox_yAxisLStickMIN, xbox_yAxisLStick, 2);
